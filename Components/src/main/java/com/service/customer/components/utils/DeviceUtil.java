...
public class DeviceUtil {
    ...
    public String getDeviceInfo(Context ctx, boolean isEncrypt, int appVersion) {
        try {
            ...
            LogUtil.getInstance().print("DeviceInfo:" + object.toString());
            if (isEncrypt) {
                return SecurityUtil.getInstance().encrypt3Des(object.toString(), Constant.Data.KEY, Constant.Data.FROMAT);
            } else {
                return object.toString();
            }
        } catch (UnsupportedEncodingException | NoSuchPaddingException | InvalidAlgorithmParameterException | NoSuchAlgorithmException | IllegalBlockSizeException | BadPaddingException | InvalidKeyException | InvalidKeySpecException e) {
            e.printStackTrace();
            return null;
        }
    }
