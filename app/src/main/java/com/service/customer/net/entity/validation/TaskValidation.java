...
public class TaskValidation extends ValidationExecutor {

    @Override
    public boolean doValidate(Context context, String text) {
        if (TextUtils.isEmpty(text)) {
            ToastUtil.getInstance().showToast(context, context.getString(R.string.task_prompt), Toast.LENGTH_SHORT);
            return false;
        } else {
            return true;
        }
